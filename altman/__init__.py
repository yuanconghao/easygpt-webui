...
from altman.llm_models import Model, ModelUtils


class ChatCompletion:
    @staticmethod
    def create(model: Model.model or str, messages: list, api_key: str = None, provider: Provider.Provider = None,
        ...
        try:
            ...
            return (
                engine._create_completion(api_key=api_key, model=model.name, messages=messages, stream=stream, **kwargs)
                if stream else ''.join(
                    engine._create_completion(api_key=api_key, model=model.name, messages=messages, stream=stream,
                                              **kwargs)))
